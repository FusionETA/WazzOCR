...
// Points a Wazzup account (identified by its API key) at our webhook URL, so
// incoming messages are delivered to webhook.php. This is per Wazzup account,
// so it covers all channels under that key. Returns { ok, error? }.
const WAZZUP_WEBHOOKS_ENDPOINT = 'https://api.wazzup24.com/v3/webhooks';
async function registerWebhook(apiKey, webhookUrl) {
  if (!apiKey) return { ok: false, error: 'Missing API key.' };
  if (!webhookUrl) return { ok: false, error: 'Missing webhook URL (set PUBLIC_WEBHOOK_URL).' };
  try {
    const resp = await fetch(WAZZUP_WEBHOOKS_ENDPOINT, {
      method: 'PATCH',
      headers: { 'Authorization': `Bearer ${apiKey}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ webhooksUri: webhookUrl, subscriptions: { messagesAndStatuses: true } })
    });
    if (!resp.ok) {
      const body = await resp.text().catch(() => '');
      return { ok: false, error: `Wazzup returned HTTP ${resp.status}: ${body.slice(0, 200)}` };
    }
    return { ok: true };
  } catch (err) {
...
module.exports = { sendMessage, sendSystemMessage, registerWebhook, systemCreds, WAZZUP_ENDPOINT };
